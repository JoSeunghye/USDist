...
import math
from losses.utils.registry_factory import LOSSES_REGISTRY
from losses.utils.dist_helper import allreduce
...
class SoftmaxEQLV2Loss(_Loss):
    ...
    def forward(self, input, label):
        if self.indicator == 'pos':
            indicator = self.pos_grad.detach().cuda()
        elif self.indicator == 'neg':
            indicator = self.neg_grad.detach().cuda()
        elif self.indicator == 'pos_and_neg':
            indicator = self.pos_neg.detach() + self.neg_grad.detach().cuda()
        else:
            raise NotImplementedError
        ...
        # print(input.is_cuda,indicator.is_cuda)
